...
`app/api/contact/route.ts` — POST endpoint for the contact form. Accepts **`multipart/form-data`** (not JSON — the form carries an optional file), validates fields server-side, escapes user input into the notification email, drops submissions that fill the `website` honeypot, and rate limits (5 requests/min per IP) via `lib/rate-limit.ts`. **Email delivery is wired up via Resend** (`RESEND_API_KEY` + `CONTACT_EMAIL`); the sending domain in the `from` address must be verified in Resend before going live. The rate limiter (`isRateLimited`) uses **Upstash Redis** when `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN` are set (shared across serverless instances) and falls back to an in-memory `Map` otherwise; if Upstash is unreachable it fails open to the in-memory limiter so an outage never blocks legitimate submissions.

The form also takes **one optional attachment**, forwarded to Resend as an email attachment (no blob storage). The shared rules live in `lib/contact-attachment.ts` and are applied on both the client and the server so the two checks can't drift: max **4 MB** (`MAX_ATTACHMENT_BYTES` — Vercel rejects request bodies over ~4.5 MB before the function is invoked) and **PDF, PNG, JPEG or WebP** only. The browser's declared MIME type is treated as untrusted: the server confirms the format by **file signature** (`sniffAttachment`) and requires the extension to agree, so a ZIP renamed to `.png` — or a real PNG named `invoice.pdf` — is rejected with a 400. Filenames are rebuilt from a sanitized stem plus the sniffed extension (`buildSafeFilename`). Note Resend's `Attachment.contentType` is camelCase; the older `content_type` spelling is silently dropped.

`app/api/subscribe/route.ts` — POST endpoint for the newsletter / lead-magnet signup (`NewsletterSignup` component). Reuses the same honeypot, `isRateLimited`, `reportError`, and lazy Resend patterns as the contact route. On a valid email it sends the subscriber a localized welcome email (built from `dict.newsletter`), notifies `CONTACT_EMAIL`, and — when `RESEND_SEGMENT_ID` (preferred) or the deprecated `RESEND_AUDIENCE_ID` is set — best-effort adds the contact to that Resend Segment/Audience (a list failure never blocks the welcome email).
...
RESEND_SEGMENT_ID=...                            # optional: newsletter subscriber list — new Resend Segments model (/api/subscribe)
RESEND_AUDIENCE_ID=...                           # optional: deprecated fallback for the legacy Resend Audiences model
NEXT_PUBLIC_CALCOM_LINK=...                       # optional: Cal.com link for the /book page (full URL or handle/slug)
```
`NEXT_PUBLIC_BASE_URL` is the single source of truth for the site origin via `siteUrl` in `lib/i18n/config.ts` (used by `lib/i18n/metadata.ts`, `app/sitemap.ts`, `app/robots.ts`, and the locale layout's `metadataBase`). The Upstash variables are optional — see the API routes section for the rate-limiter fallback behaviour. The newsletter list is optional: when `RESEND_SEGMENT_ID` is set, `/api/subscribe` adds subscribers to that Resend **Segment** (the current model); `RESEND_AUDIENCE_ID` is a deprecated fallback for the legacy **Audiences** model (segment and audience IDs are not interchangeable). `NEXT_PUBLIC_CALCOM_LINK` powers the `/book` page — when empty the page renders a contact-form fallback instead of the Cal.com embed.
